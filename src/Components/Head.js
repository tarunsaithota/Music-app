...
const Head = () => {
  ...
  const handleSearchSuggestion = async (s) => {
    const result = await fetch("https://saavn.dev/api/search/songs?query=" + s);
    const data = await result.json();
    dispatch(fetchSearchSongs(data.data.results));
        ...
        {userDetails && (
          <>
            <div className="col-span-4 px-10">
              <div>
                <input
                  className="w-3/4 md:w-1/2 border border-gray-700 md:p-1 pl-2 md:pl-4 rounded-l-full text-black "
                  type="text"
                  placeholder="Search"
                  value={searchData}
              ...
              {showSuggestions && (
                <div
                  className="absolute  text-black m-1 w-1/2 md:w-1/4"
                  ref={suggestionBoxRef}
                >
                  <ul className="bg-white rounded-lg">
                    {storeSuggestions.map((s, index) => (
                      <li
                        className="py-1 hover:bg-gray-300 rounded-lg px-2 my-1"
                        key={index}
                        onMouseDown={() => handleSearchSuggestion(s)}
                      >
                        🔍 {s}
                      </li>
                    ))}
